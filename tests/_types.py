...
from myao.containers import Torrent, User, Comment, View
...
def check_torrent_fields(torrent: Torrent) -> None:
    ...
    assert isinstance(torrent.category, Category)
    assert isinstance(torrent.subcategory, Subcategory)
    assert torrent.subcategory in torrent.category.value.subcategories
    ...
    if torrent.torrent_type:
        assert isinstance(torrent.torrent_type, TorrentType)
